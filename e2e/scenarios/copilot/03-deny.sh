...
# Invokes copilot -p headlessly with a force push prompt; verifies via audit log.
#
# Note: Copilot's exit code may be 0 even when a hook denies, because Copilot handles
# hook denials internally. The proof of denial is in the audit log, not Copilot's exit code.
...
# Sets up workspace with deny fixture, invokes copilot headlessly with force push prompt,
...
scenario_deny() {
  ...
  cp "${E2E_ROOT}/fixtures/copilot/hooks-deny.yaml" "${workspace}/.claude/hooks.yaml"
  ...
  # Invoke Copilot headlessly — don't fail on non-zero exit (deny IS the expected outcome)
  invoke_copilot_headless "${workspace}" \
