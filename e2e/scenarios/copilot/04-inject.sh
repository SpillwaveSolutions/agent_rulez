...
# Invokes copilot -p headlessly; verifies inject_command created the marker file
...
# Sets up workspace with inject template (substituting __WORKSPACE__), invokes copilot
...
scenario_inject() {
  ...
  sed "s|__WORKSPACE__|${workspace}|g" \
    "${E2E_ROOT}/fixtures/copilot/hooks-inject.yaml.template" \
  ...
  # Invoke Copilot headlessly to trigger the inject rule
  invoke_copilot_headless "${workspace}" "Run this bash command: echo hello-e2e-inject" 120 || true
