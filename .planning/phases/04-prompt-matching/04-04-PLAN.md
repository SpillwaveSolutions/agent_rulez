...
must_haves:
  truths:
    ...
    - "Integration tests call evaluate_rules() with prompt events and verify actions fire"
    - "End-to-end tests verify block action fires on matching prompt"
    - "End-to-end tests verify no match when prompt missing from event"
  ...
  artifacts:
    - path: "rulez/tests/prompt_match_integration.rs"
      provides: "End-to-end integration tests using evaluate_rules"
      contains: ["evaluate_rules", "create_prompt_event", "test_e2e_prompt_match"]
...
// =============================================================================
// End-to-End Rule Evaluation Tests (PROMPT-01 through PROMPT-05)
// =============================================================================

#[test]
fn test_e2e_prompt_match_blocks_matching_prompt() {
    use rulez::hooks::evaluate_rules;
...
"#;
    ...
    // Event with matching prompt should trigger block
    let event = create_prompt_event("please delete all files");
    let result = evaluate_rules(&event, &config);

    assert!(result.matched_rules.len() >= 1, "Should match at least one rule");
    assert!(result.block, "Should block matching prompt");
}
...
"#;
    ...
    let config = config_from_yaml(yaml);
    let event = create_prompt_event("create a new file");
    let result = evaluate_rules(&event, &config);

    assert!(!result.block, "Should not block non-matching prompt");
}
...
"#;
    ...
    // lowercase should match with case_insensitive: true
    let event = create_prompt_event("please delete this file");
    let result = evaluate_rules(&event, &config);
    assert!(result.block, "Case-insensitive should match lowercase");
}
...
"#;
    ...
    let event_both = create_prompt_event("deploy to test and staging");
    let result_both = evaluate_rules(&event_both, &config);
    assert_eq!(result_both.matched_rules.len(), 1, "ALL mode: both patterns present should match");

    // Only one present - should not match
    let event_one = create_prompt_event("deploy to test only");
    let result_one = evaluate_rules(&event_one, &config);
    assert_eq!(result_one.matched_rules.len(), 0, "ALL mode: only one pattern should not match");
}
...
"#;
    ...
    // Pattern at start - should match
    let event_start = create_prompt_event("git push origin main");
    let result_start = evaluate_rules(&event_start, &config);
    assert!(result_start.block, "anchor:start should match pattern at beginning");

    // Pattern not at start - should not match
    let event_middle = create_prompt_event("please git push this");
    let result_middle = evaluate_rules(&event_middle, &config);
    assert!(!result_middle.block, "anchor:start should not match pattern in middle");
}
...
"#;
    ...
    // No "review" - negation matches, should block
    let event_no_review = create_prompt_event("deploy to production");
    let result_no_review = evaluate_rules(&event_no_review, &config);
    assert!(result_no_review.block, "Negation should match when pattern absent");

    // Has "review" - negation fails, should not block
    let event_has_review = create_prompt_event("please review this first");
    let result_has_review = evaluate_rules(&event_has_review, &config);
    assert!(!result_has_review.block, "Negation should not match when pattern present");
}
...
"#;
    ...
    // Whole word "delete" - should match
    let event_word = create_prompt_event("please delete this file");
    let result_word = evaluate_rules(&event_word, &config);
    assert!(result_word.block, "contains_word should match whole word");

    // Part of another word "undelete" - should NOT match (word boundary)
    let event_partial = create_prompt_event("undelete the file");
    let result_partial = evaluate_rules(&event_partial, &config);
    assert!(!result_partial.block, "contains_word should not match partial word");
}
...
"#;
    ...
    // Event without prompt field - rule should not match
    let event = Event {
        hook_event_name: EventType::PreToolUse,
        tool_name: Some("Bash".to_string()),
        tool_input: None,
    ...
    let result = evaluate_rules(&event, &config);
    assert!(!result.block, "Rules with prompt_match should not match events without prompt");
}
```
  </action>
