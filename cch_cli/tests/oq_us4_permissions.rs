...
#[path = "common/mod.rs"]
mod common;
use common::{evidence_dir, fixture_path, read_fixture, TestEvidence, Timer};
...
fn test_us4_permission_request_injection() {
    ...
    fs::write(context_dir.join("explain-command.md"), "# Command Explanation\n\nThis command requires permission because...")
...
fn test_us4_file_operation_explanation() {
    ...
    let context_dir = claude_dir.join("context");
    fs::create_dir_all(&context_dir).expect("create context dir");
    fs::write(context_dir.join("explain-file-ops.md"), "# File Operation\n\nThis modifies files.")
        .expect("write context");
