...
fn test_us5_logs_command_works() {
    ...
    result.stdout(predicate::str::is_empty().not()
        .or(predicate::str::contains("No log entries")));
    
    evidence.pass(
        "Logs command executes successfully",
        timer.elapsed_ms(),
    );
...
fn test_us5_explain_command_works() {
    ...
    evidence.pass(
        "Explain command executes successfully",
        timer.elapsed_ms(),
    );
    let _ = evidence.save(&evidence_dir());
}
...
fn test_us5_validate_creates_default() {
    ...
    result.stdout(predicate::str::contains("Creating default")
...
fn test_us5_validate_existing_config() {
    ...
    result.stdout(predicate::str::contains("valid")
        .or(predicate::str::contains("Rules loaded")));
...
fn test_us5_validate_invalid_config() {
    ...
    fs::write(claude_dir.join("hooks.yaml"), "version: [invalid\nrules:\n  - bad yaml here")
